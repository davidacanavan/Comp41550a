 ...
 * (2 on my device) so it'll result in 2 calls to fire. Additionally I need the functionality to be called on every iteration
 * of the game loop if there's a button hold whereas the button has no method to check if the active state is a result 
 * of a hold or not, hence the class.
 */
