...
@implementation DTShowcaseLevel
...
+(id)level
{
    return [[self alloc] initWithTMXFile:LEVEL_NAME_HOSPITAL_SHOWCASE session:nil peerIdentifier:nil playerNumber:DEFAULT_PLAYER_NUMBER];
}

+(id)levelWithSession:(GKSession *)session peerIdentifier:(NSString *)peerIdentifier playerNumber:(int)playerNumber
{
    return [[self alloc] initWithTMXFile:LEVEL_NAME_HOSPITAL_SHOWCASE session:session peerIdentifier:peerIdentifier playerNumber:playerNumber];
}
...
-(BOOL)onTriggerEncountered:(DTTrigger *)trigger
{
    ...
    {
        NSString *text = [[_triggerObjects objectNamed:trigger.name] objectForKey:@"Text"];
        [[CCDirector sharedDirector] replaceScene:[DTIntroLayer node]];
        [HandyFunctions showTextBoxWithText:text fromLayer:(CCLayer *) self.gameLayer];
        //[self navigateBackToIntroScreenWithTitle:@"Fantastic!" andMessage:@"You didn't die... Yet!"];
        return YES;
    }
    else if ([trigger.name hasPrefix:@"Text Box"])
    {
        NSString *text = [[_triggerObjects objectNamed:trigger.name] objectForKey:@"Text"];
        [HandyFunctions showTextBoxWithText:text fromLayer:(CCLayer *) self.gameLayer];
        return YES;
